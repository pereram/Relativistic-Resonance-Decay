void eventTestRead()
{
  ...
  TH1D *hVecLength=new TH1D("hVecLength","",80,0.9,1.3);hVecLength->Sumw2();
  ...
  TCanvas *c2=new TCanvas("c2","#2",800,600);
    ...
    hVecLength->GetXaxis()->SetTitle("Mass (GeV)");
    hVecLength->GetYaxis()->SetTitle("");
    hVecLength->GetXaxis()->CenterTitle();
    hVecLength->GetYaxis()->CenterTitle();
    hVecLength->Fit("gaus");
    hVecLength->DrawCopy();
